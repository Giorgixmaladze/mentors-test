import { useContext } from "react"
import { AuthContext } from "../context/context"
import Details from "./Details"

const MovieCard = ({ arr }) => {
    const { addFavorite, openDetails,opened } = useContext(AuthContext)

    return (
    
        
        arr.map((item, index) => (
            <div onClick={()=> openDetails(index)}
                key={index}
                className="w-full max-w-xs sm:max-w-sm md:max-w-md h-auto flex flex-col items-center justify-between p-4 gap-4 shadow-lg shadow-white rounded-md hover:shadow-2xl transition duration-200 bg-amber-800"
            >
                <img className="w-40 h-60 object-cover rounded" src={item.image} alt={item.title} />
                <h2 className="text-white text-xl font-semibold text-center">{item.title}</h2>
                <p className="text-center text-white text-sm">{item.description}</p>
                <div className="flex items-center gap-3">
                    <p className="text-white">{item.rating}</p>
                    <span
                        onClick={() => addFavorite(item)}
                        className="material-symbols-outlined cursor-pointer text-xl"
                        style={{ color: item.isfavorite ? "red" : "white" }}
                    >
                        favorite
                    </span>
                </div>
            </div>
        )))
}

export default MovieCard